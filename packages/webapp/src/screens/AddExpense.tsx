...
export function AddExpense({
  ...
  const [amount, setAmount] = useState(expense && !itemized ? toAmountInput(expense.amount, expense.currency) : '');
  ...
  const [exact, setExact] = useState<Record<number, string>>(() =>
    expense && !itemized ? Object.fromEntries(expense.splits.map((s) => [s.userId, toAmountInput(s.amount, expense.currency)])) : {},
  );
  ...
  const preview = useMemo(() => {
    if (isItemized || cents == null || participants.length === 0) return { error: null, splits: [] };
    try {
  ...
  }, [isItemized, cents, currency, splitType, participants.join(','), JSON.stringify(weights), JSON.stringify(exact)]);
